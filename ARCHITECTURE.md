...
1. **Strict Dual Authentication Boundaries**: 
   ...
   - Dedicated mobile routes, authenticated `/api/v1/auth/*`, and `/api/v1/broadcasting/auth` use the isolated `mobile-api` limiter (300 requests/minute/user by default; `MOBILE_API_RATE_LIMIT_PER_MINUTE`). Generic/developer routes retain `api` at 60/minute/user. Explicit action-specific and login limits remain independent.
