...
- **Team Availability**: Administrators define per-workspace, timezone-aware weekly coverage with split and overnight windows. Availability gates new-message/handoff alerts only; missing schedules retain continuous delivery.
...
- **Per-user Master Email alerts**: Each agent may disable notifications generated by newly synchronized inbound email without stopping mailbox sync, email unread state, replies, or notifications from Omni channels. The preference suppresses database, realtime, browser-push, and mobile-push delivery for email-originated `NewMessageNotification` only and defaults on for compatibility.
...
- **Website AI Schedule**: Appearance offers Permanent or Scheduled Smart Bot answering with explicit IANA timezone and split/overnight active windows. Server-side evaluation controls generation and widget handoff state; legacy inside/outside schedules convert without behavior changes.
- **Segment AI Answering**: Channel Setup selects one workspace-wide Off, Always on, or Scheduled policy and Smart Bot for every supported WhatsApp, Messenger, Instagram DM, Telegram Business, and eBay account. Email Setup owns a separate policy shared by all mailboxes. Exact active hours are reevaluated on every inbound; human control pauses AI until resolution. Email excludes historical imports, automated mail, bounces, lists, self-mail, and attachment-only messages. Amazon, SMS, and public comments remain outside these policies.
- **Smart Bot Selection**: Smart Bots do not have a separate activation switch. A bot runs only where it is explicitly selected; Website Appearance selects exactly one bot, and missing/deleted selections fail closed.
