...
Inbound Gmail, Microsoft 365, and IMAP messages keep `messages.body` as the plain-text compatibility fallback and store sanitized rich content in `messages.payload.html_body`. Downloaded files use the existing message-media storage and are serialized as `messages.payload.attachments[]` with `name`, `url`, `mime_type`, `size`, and `type`; `has_attachments` remains true when a provider reports a source attachment that is too large or unsupported to store. The dedicated mobile email API also exposes additive top-level `html_body`, `attachments`, and `has_attachments` fields while preserving the Omnichannel-compatible `body` and `payload` contract. Native rendering remains owned by the app team.
...
6. `DELETE /app/social/posts/{post}` performs capability-checked provider deletion before local deletion. `DELETE /app/social/posts/{post}/local` is a workspace-scoped route that never calls the provider. It is available for orphaned published mappings whose connected account is unavailable, and for X-only posts, because WisperBot never deletes on X (`PublishedPostLifecycle::LOCAL_ONLY_NETWORKS`). Every link (all networks) carries `provider_attempted_at`, set just before the create request. `SocialPublisher::publishOnce()` does not resend while it is set. Drivers raise `PublishOutcomeUnknownException` from `GuardsPublishOutcome::createRequest()` on a timeout, a 5xx, or a success without an id; other failures clear the mark so the queue may retry. `provider_media` (`ProviderMediaCache`) keeps prepared media for reuse on retry: Facebook unpublished photos, Instagram containers, LinkedIn assets and X media ids. `POST /app/social/posts/{post}/publish-now` claims the post atomically, and the scheduler and delayed job both claim through `status`. See [Integrations → X](INTEGRATIONS.md#x-2026-09-24) and [Known issues](KNOWN_ISSUES.md).
