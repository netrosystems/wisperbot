...
Business-aware replies add optional `answer_origin` (`conversation`, `knowledge_base`, `business_guidance`, `trusted_research`, `live_product`, or `fallback`) and `citations` containing only a safe title and HTTPS URL. Verified product replies may additionally include bounded `product_facts` records (product, variant/SKU, price fields, currency, availability, HTTPS URL, and verification time). `body`/`reply` and `display_body` retain readable price text and Markdown source links so older widgets and SDKs remain useful when they ignore the additive metadata. App teams may render citation or product cards, but citations remain external links and choices remain plain customer text—not executable actions.
