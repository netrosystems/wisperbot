...
Managed AI credits are implemented locally alongside Knowledge Base video answers. The credits migration replaces commercial `ai_tokens_per_month` plan limits with finite `ai_credits_per_month`, adds organization/owner periods, encrypted idempotent ledger replay, workspace provider modes, and provider test state. DeepSeek is restricted to the Super Admin `llm_deepseek_default` integration and is excluded from client provider screens, routes, BYOK resolution, and automatic fallback. Deployment should begin with `AI_CREDITS_ENFORCE=false`, a configured Super Admin OpenAI key, migrations, current Vite assets, scheduler/worker restart, and ledger reconciliation before hard enforcement.
