...
- Messenger/Instagram private-message attachments must be normalized into typed inbox messages (`image`, `video`, `audio`, `document`, or `sticker`) instead of text-only payloads. Meta attachment URLs are treated as temporary provider URLs; web clients use the authenticated inbox media endpoint, while mobile payloads use short-lived signed media URLs so native image/video views can lazy-download/cache files in `message-media` without custom authorization headers. HEIC/HEIF photos from staff uploads, website-widget uploads, Meta, or WhatsApp should be converted to cached JPEG previews server-side when one supported converter is available: PHP Imagick with HEIC support, ImageMagick `magick`/`convert`, `heif-convert`, or `ffmpeg`.
