...
- One focused Knowledge Base is assigned to a Smart Bot. Exact approved FAQs may bypass generation; safe non-personalized answers may cache per published revision. By default, every answer must clear retrieval relevance and generation grounding checks; unrelated opinions, trivia, politics, news, and other unsupported topics use the configured fallback without consuming a successful generation credit. A client may explicitly enable **Answer outside the Knowledge Base** for safe general help, but company-specific facts still require Knowledge Base evidence. This answer-scope policy applies independently of the guarded-publishing rollout flag.
- Website-widget AI availability is configured in Appearance. Scheduling is off by default, preserving continuous answering. When enabled, the client defines weekly office hours, an IANA timezone, and whether AI answers inside or outside those hours; outside-hours is the recommended after-office preset. The backend evaluates the schedule for every inbound website message. Closed days mean all-day AI in outside-hours mode and all-day rest in inside-hours mode. Keyword automations and non-widget channel settings are not changed by this schedule.
