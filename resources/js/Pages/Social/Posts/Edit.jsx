...
export default function EditPost({ post, accounts, remoteLifecycle = {} }) {
        ...
        <ClientLayout title={t('social.edit_post_title')}>
            ...
            <div className="max-w-2xl space-y-6">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm text-neutral-400 mt-0.5">
                            {isRemotePublished ? t('social.edit_published_facebook_subtitle') : t('social.edit_post_subtitle')}
                        </p>
                    </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                    {props.flash?.error && (
                        <div className="rounded-lg bg-red-50 dark:bg-red-900/30 px-4 py-3 text-sm text-red-800 dark:text-red-200">{props.flash.error}</div>
                    )}
                    ...
                    {isRemotePublished && (
                        <div className="rounded-xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-800 dark:border-blue-800 dark:bg-blue-900/20 dark:text-blue-200">
                            {t('social.published_facebook_update_notice')}
                        </div>
                    )}
                            ...
                            {accounts.map((acct) => {
                                ...
                                return (
                                    <button
                                        ...
                                        } ${isRemotePublished ? 'cursor-not-allowed opacity-80' : ''}`}
                                    >
                    ...
                    {/* Media remains immutable after remote publication. */}
                    {!isRemotePublished && (
                        <div className="rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 p-5 space-y-3">
                                ...
                                <div key={i} className="flex items-start gap-2">
                                    ...
                                    <button
                                        ...
                                        onClick={() =>
                                            setData(
                                                'media_urls',
                                                (data.media_urls ?? []).filter((_, j) => j !== i),
                                            )
                                        }
                        ...
                        <div className="rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 p-5 space-y-3">
                            ...
                            {data.scheduled_at && (
                                <p className="text-xs text-neutral-400 flex items-center gap-1">
                                    ...
                                    {t('social.publishes_at', {
                                        time: formatInTz(tzLocalToUtcIso(data.scheduled_at, data.timezone), data.timezone),
                                    })}
                                </p>
                            )}
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        >
                            ...
                            {processing
                                ? t('social.saving')
                                : isRemotePublished
                                  ? t('social.update_facebook_post')
                                  : data.scheduled_at
                                    ? t('social.save_and_schedule')
