...
import { Plus, Trash2, ExternalLink, Share2, Clock, CheckCircle2, XCircle, Pencil, Send, Sparkles, Eye, Image, X, Calendar, Zap, Ban } from 'lucide-react';
...
const STATUS_META = {
    ...
    scheduled: {
        labelKey: 'social.status_scheduled',
        cls: 'bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300',
        icon: <Clock className="h-3 w-3" />,
    },
    publishing: {
        labelKey: 'social.status_publishing',
        cls: 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/40 dark:text-yellow-300',
        icon: <Send className="h-3 w-3" />,
    },
    published: {
        labelKey: 'social.status_published',
        cls: 'bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-300',
        icon: <CheckCircle2 className="h-3 w-3" />,
    },
    failed: {
        labelKey: 'social.status_failed',
                            ...
                            <div className="flex flex-wrap gap-2">
                                {targets.map((id) => {
                                    ...
                                    return (
                                        <div
                                            key={id}
                                            className="flex items-center gap-1.5 rounded-full border border-neutral-200 dark:border-neutral-700 px-2.5 py-1"
                                        >
                            ...
                            <div className="space-y-1.5">
                                {Object.entries(post.publish_results).map(([accountId, result]) => {
                                    ...
                                    return (
                                        <div
                                            key={accountId}
                                            className="flex items-center justify-between text-xs rounded-lg bg-neutral-50 dark:bg-neutral-800 px-3 py-2"
                                        >
                                            <span className="text-neutral-600 dark:text-neutral-400">
                                                {acct?.name ?? t('social.account_number', { id: accountId })}
                                            </span>
            ...
            <div className="flex flex-col flex-1 p-4 gap-3">
                ...
                {targets.length > 0 && (
                    <div className="flex flex-wrap gap-1.5">
                        ...
                        {targets.length > 3 && (
                            <span className="text-[11px] text-neutral-400 self-center">{t('social.more_count', { count: targets.length - 3 })}</span>
                        )}
                    </div>
                )}
                    ...
                    <div className="flex gap-1 flex-wrap">
                        {posts.links.map((link, i) =>
                            link.url ? (
                                ...
                                <span
                                    key={i}
                                    className="px-3 py-1.5 rounded text-sm border border-neutral-200 dark:border-neutral-700 text-neutral-300 dark:text-neutral-600 opacity-40 cursor-not-allowed"
                                    dangerouslySetInnerHTML={{ __html: link.label }}
                                />
                            ),
                        )}
